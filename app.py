...
def get_gsheet_client():
    ...
    try:
        ...
        if 'google_creds' in st.secrets:
            st.info("Using Streamlit secrets for Google Sheets.")
            creds_dict = dict(st.secrets["google_creds"]) # Convert AttrDict to dict
        ...
        st.error("Local 'credentials.json' file not found. Please add it to the app's root directory.")
        return None
...
def get_gdoc_service():
    ...
    try:
        # Check if running on Streamlit Cloud
        if 'google_creds' in st.secrets:
            st.info("Using Streamlit secrets for Google Docs.")
            creds_dict = dict(st.secrets["google_creds"]) # Convert AttrDict to dict
            doc_creds = service_account.Credentials.from_service_account_info(creds_dict, scopes=SCOPES)
        else:
            # Running locally
            st.info("Using local 'credentials.json' for Google Docs.")
